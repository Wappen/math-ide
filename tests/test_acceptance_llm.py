r"""Live-LLM acceptance tests (issue #15) — gated so CI never hits the network.

This module mirrors the RESOLUTION assertions from ``test_acceptance_e2e.py`` but
runs them against the *real* :class:`~math_ide.ontology.AnthropicResolver` instead
of the deterministic :class:`~math_ide.ontology.MockResolver`. It is the
"runnable locally" companion to the offline suite.
...
* ``ANTHROPIC_API_KEY`` present — so we never construct a client without a key.
* the ``anthropic`` SDK importable — via :func:`pytest.importorskip`.
...
pytestmark = [
    ...
    pytest.mark.skipif(
        ...
        reason="ANTHROPIC_API_KEY not set; cannot call the live API.",
    ),
]
...
def llm_resolved_doc(docling_dict: dict) -> MathDocument:
    """Ingest the fixture and resolve it with the live AnthropicResolver.
...
def test_llm_never_overwrites_formal_meaning(
    ...
    """#15.4 (LLM): formal meanings are byte-identical to pre-resolution values
    — the authoritative-meaning rule holds for the live resolver too."""
...
def test_llm_reaches_ready_or_sane_partial(
    ...
    """#15.4 (LLM): resolution settles at ``ready`` (success) or a usable
...
def test_llm_epsilon_coreference_when_resolution_succeeds(
    ...
    """#15.4 (LLM): when resolution succeeds, the convergence epsilon links to
...
def test_llm_adds_semantic_relations_when_successful(
    ...
    """#15.4 / #15.6 (LLM): a successful run adds origin=='semantic' relations
    and they surface in the Konvergenz concept card's semantic neighbourhood."""
    doc = llm_resolved_doc
    if doc.ingestion_state != "ready":
        pytest.skip("live resolver returned a partial result; nothing to assert")
